...
# ==========================================
# 核心 2：SFA 機器可讀數據庫萃取 (🔥精準 dis/dia/dim 分流)
# ==========================================
def parse_sfa_single_excel(sfa_excel_file):
    ...
    print(f"📊 正在建立【機器可讀數據庫】(無公差尺寸轉換為純數值)...")
            ...
            if val_col and geom_col:
                for _, row in df.iterrows():
                    ...
                    if val and val.lower() != 'nan':
                        ...
                        # 🔥 修復重置變數：每次都重新判斷，印章絕對不蓋錯！
                        current_base_code = type_code
                        has_tol = bool(re.search(r'[+\-±]', clean_val))
                        
                        if current_base_code == 'dis':
                            if not has_tol:
                                current_base_code = 'dim'  # 無公差的基本尺寸 (Dimension)
                            elif 'cylindrical_surface' in geom.lower() or '⌀' in clean_val:
                                current_base_code = 'dia'  # 有公差的圓柱直徑 (Diameter)
    ...
    for t in tolerance_db:
        # 🔥 依據需求：無公差的只標數值 (BOM表呈現)
        if t['code_name'].startswith('dim'):
            pmi_str = f"🔹 {t['value']}"
        elif t['code_name'] == 'dat':
            pmi_str = f"🚩 dat: {t['value']}"
        else:
            pmi_str = f"🎯 {t['code_name']}: {t['value']}"
...
def export_sfa_bom_mbd_excel(stp_file, sfa_excel_file, output_excel_path, custom_part_num):
    ...
    try:
        ...
        print(f"\n✅ 大功告成！已產出【純淨版 dis/dia/dim】的專屬清單：{output_excel_path}")
