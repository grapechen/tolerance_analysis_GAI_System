...
def parse_sfa_single_excel(sfa_excel_file):
    ...
    print(f"📊 正在精準讀取 SFA Excel 公差字典：{sfa_excel_file}...")
    try:
                ...
                for _, row in df.iterrows():
                    ...
                    if val and val.lower() != 'nan':
                        ...
                        for fid in re.findall(r'advanced_face\s+(\d+)', geom, re.IGNORECASE):
                            pmi_mapping[fid].add(f"{prefix}{clean_val}{suffix}")
                            
    except Exception as e:
        print(f"⚠️ 讀取 SFA Excel 時發生錯誤: {e}")
            
    print(f"✅ 成功提取 {len(pmi_mapping)} 個帶有公差的 Face ID！\n")
    return pmi_mapping

# ==========================================
# 核心 3：主執行引擎 (帶自訂零件編號前綴)
# ==========================================
def export_sfa_bom_mbd_excel(stp_file, sfa_excel_file, output_excel_path, custom_part_num):
    print(f"🚀 啟動【SFA ：自訂零件編號 ({custom_part_num}) 與特徵代號 (P/S/H)】...")
    ...
    def build_excel_data(pd_id, level=0, qty=1, parent_name="無 (頂層)"):
        ...
        excel_data.append({
            ...
            "幾何參數": "", "接觸配合 (Mating)": "", "SFA 對接公差 (PMI)": "", "Face ID": f"Def #{pd_id}"
        })
        ...
        if is_assembly:
            ...
            if is_noise_part(name, qty):
                excel_data.append({
                    ...
                    "幾何參數": "已過濾隱藏", "接觸配合 (Mating)": "", "SFA 對接公差 (PMI)": "", "Face ID": ""
                })
                ...
                for f_type, param_group in part_features[name].items():
                    for param_str, f_ids in param_group.items():
                        for fid in f_ids:
                            ...
                            pmi_str = " | ".join(sorted(list(pmi_mapping[str(fid)]))) if has_pmi else ""
    ...
    if root_nodes:
        ...
        for name, features in part_features.items():
            excel_data.append({
                ...
                "名稱 / 幾何類型": name, "數量 / 面數": 1, "幾何參數": "", "接觸配合 (Mating)": "", "SFA 對接公差 (PMI)": "", "Face ID": "純幾何體"
            })
                ...
                for param_str, f_ids in param_group.items():
                    for fid in f_ids:
                        ...
                        excel_data.append({
                            ...
                            "特徵代號": feat_code,
                            "名稱 / 幾何類型": f_type, "數量 / 面數": 1, "幾何參數": param_str, "接觸配合 (Mating)": "", "SFA 對接公差 (PMI)": pmi_str, "Face ID": f"#{fid}"
                        })

    try:
        with pd.ExcelWriter(output_excel_path, engine='openpyxl') as writer:
            pd.DataFrame(excel_data).to_excel(writer, index=False, sheet_name='SFA代號與公差總表')
            worksheet = writer.sheets['SFA代號與公差總表']
...
if __name__ == "__main__":
    ...
    # 🎯 在這裡跳出提示，讓您自己決定要叫什麼編號！
    print("="*50)
    user_input_num = input("📝 請輸入這個圖檔的專屬零件編號 (例如: 1, A, PartX): ")
    print("="*50)
    
    # 將使用者輸入的名稱加到 Excel 檔名中，方便管理
    output_excel = os.path.join(file_dir, f"軸承座_{user_input_num}_自訂特徵代號總表.xlsx")
